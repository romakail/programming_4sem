...
double integral (int nThreads, double startVal, double endVal, double step)
{
	...
	struct threadInfo_t wholeInformation =
	{
		.startValue = startVal,
		.finishValue = endVal,
		.step = step
	...
	{
		calcIntegralFewThreads  (nThreads, &integral, nProcs, realProcs, &wholeInformation);
	}
	else if (nThreads > nProcs / 2)
	{
		calcIntegralManyThreads (nThreads, &integral                   , &wholeInformation);
	}
...
int calcIntegralFewThreads  (int nThreads, double* integral, int nRealProcs, int* realProcs, struct threadInfo_t* info)
{
	...
	{
		threadsInformation[i].startValue   = info->startValue + (info->finishValue - info->startValue) / nThreads * i;
		threadsInformation[i].finishValue  = info->startValue + (info->finishValue - info->startValue) / nThreads * (i + 1);
		threadsInformation[i].step         = info->step;
